...
const std::string EXPERIMENT_NAME = "step_size_fixed"; // File name base
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "Testing " << active_test_datasets.size() << " datasets with " << RUNS_PER_DATASET << " runs each" << std::endl;
        ...
        for (const auto &dataset: datasets) {
            ...
            if (std::find(active_test_datasets.begin(), active_test_datasets.end(), dataset.name) == active_test_datasets.end()) {
                continue;
            }
            ...
            for (int run = start_run - 1; run < RUNS_PER_DATASET; run++) { // start_run is 1-based, loop is 0-based
                if (ENABLE_VALIDATION_OUTPUT) {
                ...
                try {
                    ...
                    if (ENABLE_VALIDATION_OUTPUT) {
                        if (run == start_run - 1) { // First run of this session
