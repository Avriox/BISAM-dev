...
public:
    ...
    void add_estimation_results(const std::string& dataset_name, const std::string& run_name,
        ...
        for (int i = 0; i < result.indicator_means.n_elem; i++) {
            double true_indicator = 0.0;

            // Check if this position has a true break
            if (dataset.true_breaks.n_rows > 0) {
                for (int j = 0; j < dataset.true_breaks.n_rows; j++) {
                    if (static_cast<int>(dataset.true_breaks(j, 1)) == i) {
                        true_indicator = 1.0;
                        break;
                    }
                }
            ...
            double error = std::abs(result.indicator_means(i) - true_indicator);
            estimation_results_.emplace_back(experiment_name_, run_name, dataset_name, run_number,
                                           "break_indicator", i, true_indicator,
                                           result.indicator_means(i), error,
                                           result.indicator_means(i));
        }
        ...
        if (result.sigma2_means.n_elem > 0) {
            estimation_results_.emplace_back(experiment_name_, run_name, dataset_name, run_number,
                                           ...
                                           std::abs(result.sigma2_means(0) - dataset.error_sd * dataset.error_sd));
        }
    }
